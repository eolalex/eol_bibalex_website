...
class UnitConversions
  @functions = [
    ...
    { starting_units: ['http://purl.obolibrary.org/obo/UO_0000195'], # farenheiht
      ending_unit: 'http://purl.obolibrary.org/obo/UO_0000027', # degrees Celsius
  ...
  class << self
    ...
    def all_starting_units
      @all_starting_units ||= @functions.flat_map {|function| function[:starting_units]}
    end
    ...
    def starting_units(uri)
      @ending_unit_to_starting_units ||= @functions.map {|function| [function[:ending_unit], function[:starting_units]]}.to_h
      @ending_unit_to_starting_units[uri]
    end

    def convert(val, units_uri)
      return([val, units_uri]) unless can_convert?(units_uri)
      function = @functions.find {|function| function[:starting_units].include? units_uri}
      [function[:function].call(val), function[:ending_unit]]
    end
  end
end
