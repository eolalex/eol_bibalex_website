  ...
  module Pages
    class V0_0 < Api::Methods
      ...
      def self.load_media(media, params, page)
        ...
        content_ids = PageContent.where("page_id = ? and content_id in (?) and content_type = ? and trust in (?) "\
          "and is_incorrect = ? and is_misidentified = ? and is_hidden = ? and is_duplicate = ?",
          page.id, media_ids, "Medium", params[:vetted_types], false, false, false, false).map(&:content_id)
      ...
      def self.promote_exemplar!(exemplar_object, existing_objects_of_same_type, options = {}, license_ids, page, type)
        ...
        content_object = PageContent.where("page_id = ? and content_id = ? and content_type = ? and is_incorrect = ? "\
          "and is_misidentified = ? and is_hidden = ? and is_duplicate = ?",
          page.id, exemplar_object.id, type, false, false, false, false).first
